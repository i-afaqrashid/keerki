<template>
  <b-modal
    id="edit-account"
    centered
    v-model="visible"
    hide-footer
    @hide="handleClose('headerClose')"
    content-class="client-class"
    header-class="header-class2"
    body-class="body-class2"
    hider-header
    size="xl"
  >
    <!-- editPermissionsRoles:"Roles",editPermissionsAddNewRoles:"Add new role",editPermissionsDuplicateSelectedRoles:"Duplicate selected role",editPermissionsDeleteSelectedRole:"Delete select role",editPermissionsUsersAreMoved:"Users are moved to default role and remain active",editPermissionsTechEmployee:"Tech Employee",editPermissionsFinancialEmploye:"Financial Employee",editPermissionsCustomerService:"Customer Service",editPermissionsPermissions:"Permissions",editPermissionsSensitive:"Sensitive",editPermissionsClientDetails:"Client details",editPermissionsName:"Name",editPermissionsAddress:"Address",editPermissionsEmail:"Email Address",editPermissionsPhone:"Phone Number",editPermissionsBalance:"Balance",editPermissionsOrders:"Orders",editPermissionsViewOrders:"View Orders",editPermissionsSendSms:"Send SMS",editPermissionsDone:"Done",editPermissionsTransfer:"Transfer",editPermissionsCancel:"Cancel",editPermissionsMessage:"Messages",editPermissionsViewMessages:"View Messages",editPermissionsReplyToMessages:"Reply to Messages",editPermissionsNotifications:"Notifications",editPermissionsView:"View",editPermissionsClientStatus:"Client status",editPermissionsMoreDetails:"More details about it",editPermissionsAccountStatus:"Account Status",
        editPermissionsEmployeesAndUserManagment:"Employees and User Managment",editPermissionsCreateUser:"Create User",editPermissionsManageUser:"Manage User",editPermissionsActivate:"Activate, suspend, & delete clients users or edit their profile.",editPermissionsUsers:"Users",editPermissionsMoveUsers:"Move users to this role",editPermissionsMoveUser:"Move user(s) back to default role",editPermissionsSearch:"Search",editPermissionsFirstLast:"First Last Name" -->
    <div class="w-100 d-flex flex-column flex-lg-row justify-content-center">
      <div
        class="w-33-resp py-3 border-right d-flex flex-column align-items-center"
      >
        <div
          class="w-25-resp align-self-start d-flex align-items-center justify-content-around"
        >
          <svg
            width="42"
            height="42"
            viewBox="0 0 42 42"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <circle cx="21" cy="21" r="21" fill="#0074BD" />
            <path
              d="M29.75 20.25C31.4688 20.25 32.875 18.8828 32.875 17.125C32.875 15.4062 31.4688 14 29.75 14C27.9922 14 26.625 15.4062 26.625 17.125C26.625 18.8828 27.9922 20.25 29.75 20.25ZM29.75 15.875C30.4141 15.875 31 16.4609 31 17.125C31 17.8281 30.4141 18.375 29.75 18.375C29.0469 18.375 28.5 17.8281 28.5 17.125C28.5 16.4609 29.0469 15.875 29.75 15.875ZM12.25 20.25C13.9688 20.25 15.375 18.8828 15.375 17.125C15.375 15.4062 13.9688 14 12.25 14C10.4922 14 9.125 15.4062 9.125 17.125C9.125 18.8828 10.4922 20.25 12.25 20.25ZM12.25 15.875C12.9141 15.875 13.5 16.4609 13.5 17.125C13.5 17.8281 12.9141 18.375 12.25 18.375C11.5469 18.375 11 17.8281 11 17.125C11 16.4609 11.5469 15.875 12.25 15.875ZM27.7188 24.1172C26.625 22.5547 24.5547 21.9297 22.7188 22.5156C22.1719 22.6719 21.5859 22.75 20.9609 22.75C20.375 22.75 19.7891 22.6719 19.2422 22.5156C17.4062 21.9297 15.2969 22.5156 14.2422 24.1172C13.7734 24.8594 13.4609 25.7188 13.4609 26.6562V28.375C13.4609 29.4297 14.3203 30.25 15.3359 30.25H26.5859C27.6406 30.25 28.4609 29.4297 28.4609 28.375V26.6562C28.5 25.7188 28.1875 24.8594 27.7188 24.1172ZM26.625 28.375H15.375V26.6562C15.375 25.2109 16.5078 24.0391 17.9141 24C18.8906 24.4297 19.9453 24.625 21 24.625C22.0156 24.625 23.0703 24.4297 24.0469 24C25.4531 24.0391 26.625 25.2109 26.625 26.6562V28.375ZM30.2188 21.5H29.2812C28.5781 21.5 27.9531 21.7344 27.4453 22.0859C27.9531 22.4766 28.4219 22.9453 28.7734 23.4922C28.9297 23.4531 29.0859 23.375 29.2422 23.375H30.1797C30.9609 23.375 31.5859 24.0391 31.5859 24.7812C31.5859 25.3281 32.0156 25.7188 32.5234 25.7188C33.0703 25.7188 33.4609 25.3281 33.4609 24.7812C33.5 22.9844 32.0156 21.5 30.2188 21.5ZM21 21.5C23.3828 21.5 25.375 19.5469 25.375 17.125C25.375 14.7422 23.3828 12.75 21 12.75C18.5781 12.75 16.625 14.7422 16.625 17.125C16.625 19.5469 18.5781 21.5 21 21.5ZM21 14.625C22.3672 14.625 23.5 15.7578 23.5 17.125C23.5 18.5312 22.3672 19.625 21 19.625C19.5938 19.625 18.5 18.5312 18.5 17.125C18.5 15.7578 19.5938 14.625 21 14.625ZM14.5156 22.0859C14.0078 21.7344 13.3828 21.5 12.7188 21.5H11.7812C9.94531 21.5 8.5 22.9844 8.5 24.7812C8.5 25.3281 8.89062 25.7188 9.4375 25.7188C9.94531 25.7188 10.375 25.3281 10.375 24.7812C10.375 24.0391 11 23.375 11.7812 23.375H12.7188C12.875 23.375 13.0312 23.4531 13.1875 23.4922C13.5391 22.9453 14.0078 22.4766 14.5156 22.0859Z"
              fill="white"
            />
          </svg>
          <p class="mb-0">{{ $t("editPermissionsRoles") }}</p>
        </div>
        <div
          class="mt-4 w-70 align-self-start d-flex align-items-center justify-content-around"
        >
          <div class="d-flex flex-column">
            <button class="outline-none">
              <svg
                width="14"
                height="15"
                viewBox="0 0 14 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M10.5 7.3125C10.5 7.14844 10.3359 6.98438 10.1719 6.98438H7.76562V4.57812C7.76562 4.41406 7.60156 4.25 7.4375 4.25H6.5625C6.37109 4.25 6.23438 4.41406 6.23438 4.57812V6.98438H3.82812C3.63672 6.98438 3.5 7.14844 3.5 7.3125V8.1875C3.5 8.37891 3.63672 8.51562 3.82812 8.51562H6.23438V10.9219C6.23438 11.1133 6.37109 11.25 6.5625 11.25H7.4375C7.60156 11.25 7.76562 11.1133 7.76562 10.9219V8.51562H10.1719C10.3359 8.51562 10.5 8.37891 10.5 8.1875V7.3125ZM13.7812 7.75C13.7812 4.00391 10.7461 0.96875 7 0.96875C3.25391 0.96875 0.21875 4.00391 0.21875 7.75C0.21875 11.4961 3.25391 14.5312 7 14.5312C10.7461 14.5312 13.7812 11.4961 13.7812 7.75ZM12.4688 7.75C12.4688 10.7852 10.0078 13.2188 7 13.2188C3.96484 13.2188 1.53125 10.7852 1.53125 7.75C1.53125 4.74219 3.96484 2.28125 7 2.28125C10.0078 2.28125 12.4688 4.74219 12.4688 7.75Z"
                  fill="#0074BD"
                />
              </svg>
            </button>
            <button class="outline-none">
              <svg
                width="13"
                height="15"
                viewBox="0 0 13 15"
                fill="none"
                class="mt-3"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M11.8398 2.55469L10.4453 1.16016C10.2266 0.941406 9.81641 0.75 9.51562 0.75H4.8125C4.07422 0.75 3.5 1.35156 3.5 2.0625V3.375H1.3125C0.574219 3.375 0 3.97656 0 4.6875V13.4375C0 14.1758 0.574219 14.75 1.3125 14.75H7.4375C8.14844 14.75 8.75 14.1758 8.75 13.4375V12.125H10.9375C11.6484 12.125 12.25 11.5508 12.25 10.8125V3.48438C12.25 3.18359 12.0586 2.77344 11.8398 2.55469ZM7.27344 13.4375H1.47656C1.36719 13.4375 1.3125 13.3828 1.3125 13.2734V4.85156C1.3125 4.76953 1.36719 4.6875 1.47656 4.6875H3.5V10.8125C3.5 11.5508 4.07422 12.125 4.8125 12.125H7.4375V13.2734C7.4375 13.3828 7.35547 13.4375 7.27344 13.4375ZM10.7734 10.8125H4.97656C4.86719 10.8125 4.8125 10.7578 4.8125 10.6484V2.22656C4.8125 2.14453 4.86719 2.0625 4.97656 2.0625H7.875V4.46875C7.875 4.85156 8.14844 5.125 8.53125 5.125H10.9375V10.6484C10.9375 10.7578 10.8555 10.8125 10.7734 10.8125ZM10.9375 3.8125H9.1875V2.0625H9.43359C9.48828 2.0625 9.51562 2.08984 9.54297 2.11719L10.8828 3.45703C10.9102 3.48438 10.9375 3.51172 10.9375 3.56641V3.8125Z"
                  fill="#0074BD"
                />
              </svg>
            </button>
            <button class="outline-none">
              <svg
                width="14"
                height="15"
                viewBox="0 0 14 15"
                fill="none"
                class="mt-3 opacity-50"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M3.82812 8.51562H10.1719C10.3359 8.51562 10.5 8.37891 10.5 8.1875V7.3125C10.5 7.14844 10.3359 6.98438 10.1719 6.98438H3.82812C3.63672 6.98438 3.5 7.14844 3.5 7.3125V8.1875C3.5 8.37891 3.63672 8.51562 3.82812 8.51562ZM13.7812 7.75C13.7812 4.00391 10.7461 0.96875 7 0.96875C3.25391 0.96875 0.21875 4.00391 0.21875 7.75C0.21875 11.4961 3.25391 14.5312 7 14.5312C10.7461 14.5312 13.7812 11.4961 13.7812 7.75ZM12.4688 7.75C12.4688 10.7852 10.0078 13.2188 7 13.2188C3.96484 13.2188 1.53125 10.7852 1.53125 7.75C1.53125 4.74219 3.96484 2.28125 7 2.28125C10.0078 2.28125 12.4688 4.74219 12.4688 7.75Z"
                  fill="#FF3A67"
                />
              </svg>
            </button>
          </div>
          <div class="d-flex flex-column justify-content-center">
            <p class="mb-0">{{ $t("editPermissionsAddNewRoles") }}</p>
            <p class="mb-0 mt-2">
              {{ $t("editPermissionsDuplicateSelectedRoles") }}
            </p>
            <p class="mb-0 mt-2 pink-text opacity-50">
              {{ $t("editPermissionsDeleteSelectedRole") }}
            </p>
          </div>
        </div>
        <div class="w-70 mt-1">
          <p class="mb-0 fs-10 text-muted text-center">
            {{ $t("editPermissionsUsersAreMoved") }}
          </p>
        </div>
        <div class="w-90 mt-4">
          <button
            class="w-70 bg-primary text-white d-flex justify-content-start align-items-center btn rounded-0"
          >
            {{ $t("editPermissionsTechEmployee") }}
          </button>
        </div>
        <div class="w-90 mt-2">
          <button
            class="w-100 d-flex justify-content-start align-items-center btn rounded-0"
          >
            {{ $t("editPermissionsFinancialEmployee") }}
          </button>
        </div>
        <div class="w-90 mt-2">
          <button
            class="w-100 d-flex justify-content-start align-items-center btn rounded-0"
          >
            {{ $t("editPermissionsCustomerService") }}
          </button>
        </div>
      </div>
      <div
        class="w-33-resp mt-3 mt-lg-0 vh-100 overflow-scroll py-3 border-right d-flex flex-column align-items-center"
      >
        <div
          class="w-25-resp align-self-start d-flex align-items-center justify-content-around"
        >
          <svg
            width="42"
            height="42"
            viewBox="0 0 42 42"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <circle cx="21" cy="21" r="21" fill="#0074BD" />
            <path
              d="M27.875 19H26.625V17.2031C26.625 14.0781 24.125 11.5391 21 11.5C17.875 11.5 15.375 14.0391 15.375 17.125V19H14.125C13.0703 19 12.25 19.8594 12.25 20.875V29.625C12.25 30.6797 13.0703 31.5 14.125 31.5H27.875C28.8906 31.5 29.75 30.6797 29.75 29.625V20.875C29.75 19.8594 28.8906 19 27.875 19ZM17.25 17.125C17.25 15.0938 18.9297 13.375 21 13.375C23.0312 13.375 24.75 15.0938 24.75 17.125V19H17.25V17.125ZM27.875 29.625H14.125V20.875H27.875V29.625Z"
              fill="white"
            />
          </svg>

          <p class="mb-0">{{ $t("editPermissionsPermissions") }}</p>
        </div>
        <div class="w-90 mt-3">
          <p class="text-muted">{{ $t("editPermissionsSensitive") }}</p>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsClientDetails") }}</p>
          <button class="d-flex align-items-center outline-none">
            <svg
              width="19"
              height="15"
              viewBox="0 0 19 15"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18.3359 13.6289L1.98438 0.859375C1.90234 0.804688 1.79297 0.75 1.71094 0.75C1.57422 0.75 1.4375 0.832031 1.35547 0.914062L1.08203 1.26953C1.02734 1.32422 0.972656 1.46094 0.972656 1.54297C0.972656 1.65234 1.05469 1.81641 1.16406 1.87109L17.5156 14.668C17.5703 14.7227 17.6797 14.75 17.7891 14.75C17.8984 14.75 18.0352 14.6953 18.1172 14.5859L18.3906 14.2578C18.4453 14.2031 18.5 14.0664 18.5 13.9844C18.5 13.875 18.418 13.7109 18.3359 13.6289ZM9.09375 4.76953L12.7852 7.64062C12.7305 6 11.3906 4.6875 9.75 4.6875C9.55859 4.71484 9.28516 4.74219 9.09375 4.76953ZM10.3789 10.7578L6.6875 7.88672C6.74219 9.52734 8.08203 10.8125 9.75 10.8125C9.91406 10.8125 10.1875 10.7852 10.3789 10.7578ZM9.75 3.8125C12.4297 3.8125 14.918 5.31641 16.2305 7.75C15.957 8.26953 15.4375 9.00781 15.0273 9.41797L16.0664 10.2109C16.5586 9.69141 17.1875 8.78906 17.5156 8.16016C17.5703 8.05078 17.625 7.88672 17.625 7.75C17.625 7.64062 17.5703 7.47656 17.5156 7.36719C16.0391 4.46875 13.0859 2.5 9.75 2.5C8.73828 2.5 7.78125 2.69141 6.87891 3.01953L8.13672 4.03125C8.65625 3.89453 9.20312 3.8125 9.75 3.8125ZM9.75 11.6875C7.04297 11.6875 4.55469 10.1836 3.24219 7.75C3.51562 7.25781 4.03516 6.51953 4.44531 6.10938L3.40625 5.31641C2.91406 5.83594 2.28516 6.73828 1.95703 7.36719C1.90234 7.47656 1.875 7.64062 1.875 7.77734C1.875 7.88672 1.90234 8.05078 1.95703 8.16016C3.43359 11.0586 6.38672 13 9.75 13C10.7344 13 11.6914 12.8086 12.5938 12.5078L11.3359 11.4961C10.8164 11.6328 10.2695 11.6875 9.75 11.6875Z"
                fill="#0074BD"
              />
            </svg>

            <AdminPopUp />
          </button>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsName") }}</p>
          <button class="d-flex align-items-center outline-none">
            <svg
              width="19"
              height="15"
              viewBox="0 0 19 15"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18.3359 13.6289L1.98438 0.859375C1.90234 0.804688 1.79297 0.75 1.71094 0.75C1.57422 0.75 1.4375 0.832031 1.35547 0.914062L1.08203 1.26953C1.02734 1.32422 0.972656 1.46094 0.972656 1.54297C0.972656 1.65234 1.05469 1.81641 1.16406 1.87109L17.5156 14.668C17.5703 14.7227 17.6797 14.75 17.7891 14.75C17.8984 14.75 18.0352 14.6953 18.1172 14.5859L18.3906 14.2578C18.4453 14.2031 18.5 14.0664 18.5 13.9844C18.5 13.875 18.418 13.7109 18.3359 13.6289ZM9.09375 4.76953L12.7852 7.64062C12.7305 6 11.3906 4.6875 9.75 4.6875C9.55859 4.71484 9.28516 4.74219 9.09375 4.76953ZM10.3789 10.7578L6.6875 7.88672C6.74219 9.52734 8.08203 10.8125 9.75 10.8125C9.91406 10.8125 10.1875 10.7852 10.3789 10.7578ZM9.75 3.8125C12.4297 3.8125 14.918 5.31641 16.2305 7.75C15.957 8.26953 15.4375 9.00781 15.0273 9.41797L16.0664 10.2109C16.5586 9.69141 17.1875 8.78906 17.5156 8.16016C17.5703 8.05078 17.625 7.88672 17.625 7.75C17.625 7.64062 17.5703 7.47656 17.5156 7.36719C16.0391 4.46875 13.0859 2.5 9.75 2.5C8.73828 2.5 7.78125 2.69141 6.87891 3.01953L8.13672 4.03125C8.65625 3.89453 9.20312 3.8125 9.75 3.8125ZM9.75 11.6875C7.04297 11.6875 4.55469 10.1836 3.24219 7.75C3.51562 7.25781 4.03516 6.51953 4.44531 6.10938L3.40625 5.31641C2.91406 5.83594 2.28516 6.73828 1.95703 7.36719C1.90234 7.47656 1.875 7.64062 1.875 7.77734C1.875 7.88672 1.90234 8.05078 1.95703 8.16016C3.43359 11.0586 6.38672 13 9.75 13C10.7344 13 11.6914 12.8086 12.5938 12.5078L11.3359 11.4961C10.8164 11.6328 10.2695 11.6875 9.75 11.6875Z"
                fill="#0074BD"
              />
            </svg>
            <AdminPopUp />
          </button>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsAddress") }}</p>
          <button class="outline-none d-flex align-items-center">
            <svg
              width="19"
              height="15"
              viewBox="0 0 19 15"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18.3359 13.6289L1.98438 0.859375C1.90234 0.804688 1.79297 0.75 1.71094 0.75C1.57422 0.75 1.4375 0.832031 1.35547 0.914062L1.08203 1.26953C1.02734 1.32422 0.972656 1.46094 0.972656 1.54297C0.972656 1.65234 1.05469 1.81641 1.16406 1.87109L17.5156 14.668C17.5703 14.7227 17.6797 14.75 17.7891 14.75C17.8984 14.75 18.0352 14.6953 18.1172 14.5859L18.3906 14.2578C18.4453 14.2031 18.5 14.0664 18.5 13.9844C18.5 13.875 18.418 13.7109 18.3359 13.6289ZM9.09375 4.76953L12.7852 7.64062C12.7305 6 11.3906 4.6875 9.75 4.6875C9.55859 4.71484 9.28516 4.74219 9.09375 4.76953ZM10.3789 10.7578L6.6875 7.88672C6.74219 9.52734 8.08203 10.8125 9.75 10.8125C9.91406 10.8125 10.1875 10.7852 10.3789 10.7578ZM9.75 3.8125C12.4297 3.8125 14.918 5.31641 16.2305 7.75C15.957 8.26953 15.4375 9.00781 15.0273 9.41797L16.0664 10.2109C16.5586 9.69141 17.1875 8.78906 17.5156 8.16016C17.5703 8.05078 17.625 7.88672 17.625 7.75C17.625 7.64062 17.5703 7.47656 17.5156 7.36719C16.0391 4.46875 13.0859 2.5 9.75 2.5C8.73828 2.5 7.78125 2.69141 6.87891 3.01953L8.13672 4.03125C8.65625 3.89453 9.20312 3.8125 9.75 3.8125ZM9.75 11.6875C7.04297 11.6875 4.55469 10.1836 3.24219 7.75C3.51562 7.25781 4.03516 6.51953 4.44531 6.10938L3.40625 5.31641C2.91406 5.83594 2.28516 6.73828 1.95703 7.36719C1.90234 7.47656 1.875 7.64062 1.875 7.77734C1.875 7.88672 1.90234 8.05078 1.95703 8.16016C3.43359 11.0586 6.38672 13 9.75 13C10.7344 13 11.6914 12.8086 12.5938 12.5078L11.3359 11.4961C10.8164 11.6328 10.2695 11.6875 9.75 11.6875Z"
                fill="#0074BD"
              />
            </svg>
            <AdminPopUp />
          </button>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsEmail") }}</p>
          <button class="outline-none d-flex align-items-center">
            <svg
              width="19"
              height="15"
              viewBox="0 0 19 15"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18.3359 13.6289L1.98438 0.859375C1.90234 0.804688 1.79297 0.75 1.71094 0.75C1.57422 0.75 1.4375 0.832031 1.35547 0.914062L1.08203 1.26953C1.02734 1.32422 0.972656 1.46094 0.972656 1.54297C0.972656 1.65234 1.05469 1.81641 1.16406 1.87109L17.5156 14.668C17.5703 14.7227 17.6797 14.75 17.7891 14.75C17.8984 14.75 18.0352 14.6953 18.1172 14.5859L18.3906 14.2578C18.4453 14.2031 18.5 14.0664 18.5 13.9844C18.5 13.875 18.418 13.7109 18.3359 13.6289ZM9.09375 4.76953L12.7852 7.64062C12.7305 6 11.3906 4.6875 9.75 4.6875C9.55859 4.71484 9.28516 4.74219 9.09375 4.76953ZM10.3789 10.7578L6.6875 7.88672C6.74219 9.52734 8.08203 10.8125 9.75 10.8125C9.91406 10.8125 10.1875 10.7852 10.3789 10.7578ZM9.75 3.8125C12.4297 3.8125 14.918 5.31641 16.2305 7.75C15.957 8.26953 15.4375 9.00781 15.0273 9.41797L16.0664 10.2109C16.5586 9.69141 17.1875 8.78906 17.5156 8.16016C17.5703 8.05078 17.625 7.88672 17.625 7.75C17.625 7.64062 17.5703 7.47656 17.5156 7.36719C16.0391 4.46875 13.0859 2.5 9.75 2.5C8.73828 2.5 7.78125 2.69141 6.87891 3.01953L8.13672 4.03125C8.65625 3.89453 9.20312 3.8125 9.75 3.8125ZM9.75 11.6875C7.04297 11.6875 4.55469 10.1836 3.24219 7.75C3.51562 7.25781 4.03516 6.51953 4.44531 6.10938L3.40625 5.31641C2.91406 5.83594 2.28516 6.73828 1.95703 7.36719C1.90234 7.47656 1.875 7.64062 1.875 7.77734C1.875 7.88672 1.90234 8.05078 1.95703 8.16016C3.43359 11.0586 6.38672 13 9.75 13C10.7344 13 11.6914 12.8086 12.5938 12.5078L11.3359 11.4961C10.8164 11.6328 10.2695 11.6875 9.75 11.6875Z"
                fill="#0074BD"
              />
            </svg>
            <AdminPopUp />
          </button>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsPhone") }}</p>
          <button class="outline-none d-flex align-items-center">
            <svg
              width="19"
              height="15"
              viewBox="0 0 19 15"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18.3359 13.6289L1.98438 0.859375C1.90234 0.804688 1.79297 0.75 1.71094 0.75C1.57422 0.75 1.4375 0.832031 1.35547 0.914062L1.08203 1.26953C1.02734 1.32422 0.972656 1.46094 0.972656 1.54297C0.972656 1.65234 1.05469 1.81641 1.16406 1.87109L17.5156 14.668C17.5703 14.7227 17.6797 14.75 17.7891 14.75C17.8984 14.75 18.0352 14.6953 18.1172 14.5859L18.3906 14.2578C18.4453 14.2031 18.5 14.0664 18.5 13.9844C18.5 13.875 18.418 13.7109 18.3359 13.6289ZM9.09375 4.76953L12.7852 7.64062C12.7305 6 11.3906 4.6875 9.75 4.6875C9.55859 4.71484 9.28516 4.74219 9.09375 4.76953ZM10.3789 10.7578L6.6875 7.88672C6.74219 9.52734 8.08203 10.8125 9.75 10.8125C9.91406 10.8125 10.1875 10.7852 10.3789 10.7578ZM9.75 3.8125C12.4297 3.8125 14.918 5.31641 16.2305 7.75C15.957 8.26953 15.4375 9.00781 15.0273 9.41797L16.0664 10.2109C16.5586 9.69141 17.1875 8.78906 17.5156 8.16016C17.5703 8.05078 17.625 7.88672 17.625 7.75C17.625 7.64062 17.5703 7.47656 17.5156 7.36719C16.0391 4.46875 13.0859 2.5 9.75 2.5C8.73828 2.5 7.78125 2.69141 6.87891 3.01953L8.13672 4.03125C8.65625 3.89453 9.20312 3.8125 9.75 3.8125ZM9.75 11.6875C7.04297 11.6875 4.55469 10.1836 3.24219 7.75C3.51562 7.25781 4.03516 6.51953 4.44531 6.10938L3.40625 5.31641C2.91406 5.83594 2.28516 6.73828 1.95703 7.36719C1.90234 7.47656 1.875 7.64062 1.875 7.77734C1.875 7.88672 1.90234 8.05078 1.95703 8.16016C3.43359 11.0586 6.38672 13 9.75 13C10.7344 13 11.6914 12.8086 12.5938 12.5078L11.3359 11.4961C10.8164 11.6328 10.2695 11.6875 9.75 11.6875Z"
                fill="#0074BD"
              />
            </svg>
            <AdminPopUp />
          </button>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsBalance") }}</p>
          <button class="outline-none d-flex align-items-center">
            <svg
              width="16"
              height="11"
              viewBox="0 0 16 11"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M7.875 2.6875C7.62891 2.71484 7.24609 2.76953 7 2.82422C7.10938 3.01562 7.19141 3.37109 7.21875 3.5625C7.21875 4.41016 6.50781 5.09375 5.6875 5.09375C5.46875 5.09375 5.11328 5.01172 4.94922 4.90234C4.86719 5.14844 4.8125 5.50391 4.8125 5.75C4.8125 7.44531 6.17969 8.8125 7.875 8.8125C9.57031 8.8125 10.9375 7.44531 10.9375 5.75C10.9375 4.08203 9.57031 2.6875 7.875 2.6875ZM15.6406 5.36719C14.1641 2.46875 11.2109 0.5 7.875 0.5C4.51172 0.5 1.55859 2.46875 0.0820312 5.36719C0.0273438 5.47656 0 5.64062 0 5.77734C0 5.88672 0.0273438 6.05078 0.0820312 6.16016C1.55859 9.05859 4.51172 11 7.875 11C11.2109 11 14.1641 9.05859 15.6406 6.16016C15.6953 6.05078 15.7227 5.88672 15.7227 5.75C15.7227 5.64062 15.6953 5.47656 15.6406 5.36719ZM7.875 9.6875C5.16797 9.6875 2.67969 8.18359 1.36719 5.75C2.67969 3.31641 5.16797 1.8125 7.875 1.8125C10.5547 1.8125 13.043 3.31641 14.3555 5.75C13.043 8.18359 10.5547 9.6875 7.875 9.6875Z"
                fill="#0074BD"
              />
            </svg>

            <AdminPopUp />
          </button>
        </div>
        <div class="w-90 mt-3">
          <p class="text-muted">{{ $t("editPermissionsOrders") }}</p>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsViewOrders") }}</p>
          <button class="outline-none d-flex align-items-center">
            <svg
              width="16"
              height="11"
              viewBox="0 0 16 11"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M7.875 2.6875C7.62891 2.71484 7.24609 2.76953 7 2.82422C7.10938 3.01562 7.19141 3.37109 7.21875 3.5625C7.21875 4.41016 6.50781 5.09375 5.6875 5.09375C5.46875 5.09375 5.11328 5.01172 4.94922 4.90234C4.86719 5.14844 4.8125 5.50391 4.8125 5.75C4.8125 7.44531 6.17969 8.8125 7.875 8.8125C9.57031 8.8125 10.9375 7.44531 10.9375 5.75C10.9375 4.08203 9.57031 2.6875 7.875 2.6875ZM15.6406 5.36719C14.1641 2.46875 11.2109 0.5 7.875 0.5C4.51172 0.5 1.55859 2.46875 0.0820312 5.36719C0.0273438 5.47656 0 5.64062 0 5.77734C0 5.88672 0.0273438 6.05078 0.0820312 6.16016C1.55859 9.05859 4.51172 11 7.875 11C11.2109 11 14.1641 9.05859 15.6406 6.16016C15.6953 6.05078 15.7227 5.88672 15.7227 5.75C15.7227 5.64062 15.6953 5.47656 15.6406 5.36719ZM7.875 9.6875C5.16797 9.6875 2.67969 8.18359 1.36719 5.75C2.67969 3.31641 5.16797 1.8125 7.875 1.8125C10.5547 1.8125 13.043 3.31641 14.3555 5.75C13.043 8.18359 10.5547 9.6875 7.875 9.6875Z"
                fill="#0074BD"
              />
            </svg>
            <AdminPopUp />
          </button>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsSendSms") }}</p>
          <div class="d-flex align-items-center">
            <svg
              width="16"
              height="11"
              viewBox="0 0 16 11"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M7.875 2.6875C7.62891 2.71484 7.24609 2.76953 7 2.82422C7.10938 3.01562 7.19141 3.37109 7.21875 3.5625C7.21875 4.41016 6.50781 5.09375 5.6875 5.09375C5.46875 5.09375 5.11328 5.01172 4.94922 4.90234C4.86719 5.14844 4.8125 5.50391 4.8125 5.75C4.8125 7.44531 6.17969 8.8125 7.875 8.8125C9.57031 8.8125 10.9375 7.44531 10.9375 5.75C10.9375 4.08203 9.57031 2.6875 7.875 2.6875ZM15.6406 5.36719C14.1641 2.46875 11.2109 0.5 7.875 0.5C4.51172 0.5 1.55859 2.46875 0.0820312 5.36719C0.0273438 5.47656 0 5.64062 0 5.77734C0 5.88672 0.0273438 6.05078 0.0820312 6.16016C1.55859 9.05859 4.51172 11 7.875 11C11.2109 11 14.1641 9.05859 15.6406 6.16016C15.6953 6.05078 15.7227 5.88672 15.7227 5.75C15.7227 5.64062 15.6953 5.47656 15.6406 5.36719ZM7.875 9.6875C5.16797 9.6875 2.67969 8.18359 1.36719 5.75C2.67969 3.31641 5.16797 1.8125 7.875 1.8125C10.5547 1.8125 13.043 3.31641 14.3555 5.75C13.043 8.18359 10.5547 9.6875 7.875 9.6875Z"
                fill="#0074BD"
              />
            </svg>
            <AdminPopUp />
          </div>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsDone") }}</p>
          <div class="d-flex align-items-center">
            <svg
              width="16"
              height="11"
              viewBox="0 0 16 11"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M7.875 2.6875C7.62891 2.71484 7.24609 2.76953 7 2.82422C7.10938 3.01562 7.19141 3.37109 7.21875 3.5625C7.21875 4.41016 6.50781 5.09375 5.6875 5.09375C5.46875 5.09375 5.11328 5.01172 4.94922 4.90234C4.86719 5.14844 4.8125 5.50391 4.8125 5.75C4.8125 7.44531 6.17969 8.8125 7.875 8.8125C9.57031 8.8125 10.9375 7.44531 10.9375 5.75C10.9375 4.08203 9.57031 2.6875 7.875 2.6875ZM15.6406 5.36719C14.1641 2.46875 11.2109 0.5 7.875 0.5C4.51172 0.5 1.55859 2.46875 0.0820312 5.36719C0.0273438 5.47656 0 5.64062 0 5.77734C0 5.88672 0.0273438 6.05078 0.0820312 6.16016C1.55859 9.05859 4.51172 11 7.875 11C11.2109 11 14.1641 9.05859 15.6406 6.16016C15.6953 6.05078 15.7227 5.88672 15.7227 5.75C15.7227 5.64062 15.6953 5.47656 15.6406 5.36719ZM7.875 9.6875C5.16797 9.6875 2.67969 8.18359 1.36719 5.75C2.67969 3.31641 5.16797 1.8125 7.875 1.8125C10.5547 1.8125 13.043 3.31641 14.3555 5.75C13.043 8.18359 10.5547 9.6875 7.875 9.6875Z"
                fill="#0074BD"
              />
            </svg>
            <AdminPopUp />
          </div>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsTransfer") }}</p>
          <div class="d-flex align-items-center">
            <svg
              width="16"
              height="11"
              viewBox="0 0 16 11"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M7.875 2.6875C7.62891 2.71484 7.24609 2.76953 7 2.82422C7.10938 3.01562 7.19141 3.37109 7.21875 3.5625C7.21875 4.41016 6.50781 5.09375 5.6875 5.09375C5.46875 5.09375 5.11328 5.01172 4.94922 4.90234C4.86719 5.14844 4.8125 5.50391 4.8125 5.75C4.8125 7.44531 6.17969 8.8125 7.875 8.8125C9.57031 8.8125 10.9375 7.44531 10.9375 5.75C10.9375 4.08203 9.57031 2.6875 7.875 2.6875ZM15.6406 5.36719C14.1641 2.46875 11.2109 0.5 7.875 0.5C4.51172 0.5 1.55859 2.46875 0.0820312 5.36719C0.0273438 5.47656 0 5.64062 0 5.77734C0 5.88672 0.0273438 6.05078 0.0820312 6.16016C1.55859 9.05859 4.51172 11 7.875 11C11.2109 11 14.1641 9.05859 15.6406 6.16016C15.6953 6.05078 15.7227 5.88672 15.7227 5.75C15.7227 5.64062 15.6953 5.47656 15.6406 5.36719ZM7.875 9.6875C5.16797 9.6875 2.67969 8.18359 1.36719 5.75C2.67969 3.31641 5.16797 1.8125 7.875 1.8125C10.5547 1.8125 13.043 3.31641 14.3555 5.75C13.043 8.18359 10.5547 9.6875 7.875 9.6875Z"
                fill="#0074BD"
              />
            </svg>
            <AdminPopUp />
          </div>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsCancel") }}</p>
          <div class="d-flex align-items-center">
            <svg
              width="16"
              height="11"
              viewBox="0 0 16 11"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M7.875 2.6875C7.62891 2.71484 7.24609 2.76953 7 2.82422C7.10938 3.01562 7.19141 3.37109 7.21875 3.5625C7.21875 4.41016 6.50781 5.09375 5.6875 5.09375C5.46875 5.09375 5.11328 5.01172 4.94922 4.90234C4.86719 5.14844 4.8125 5.50391 4.8125 5.75C4.8125 7.44531 6.17969 8.8125 7.875 8.8125C9.57031 8.8125 10.9375 7.44531 10.9375 5.75C10.9375 4.08203 9.57031 2.6875 7.875 2.6875ZM15.6406 5.36719C14.1641 2.46875 11.2109 0.5 7.875 0.5C4.51172 0.5 1.55859 2.46875 0.0820312 5.36719C0.0273438 5.47656 0 5.64062 0 5.77734C0 5.88672 0.0273438 6.05078 0.0820312 6.16016C1.55859 9.05859 4.51172 11 7.875 11C11.2109 11 14.1641 9.05859 15.6406 6.16016C15.6953 6.05078 15.7227 5.88672 15.7227 5.75C15.7227 5.64062 15.6953 5.47656 15.6406 5.36719ZM7.875 9.6875C5.16797 9.6875 2.67969 8.18359 1.36719 5.75C2.67969 3.31641 5.16797 1.8125 7.875 1.8125C10.5547 1.8125 13.043 3.31641 14.3555 5.75C13.043 8.18359 10.5547 9.6875 7.875 9.6875Z"
                fill="#0074BD"
              />
            </svg>
            <AdminPopUp />
          </div>
        </div>
        <div
          class="w-90 d-flex justify-content-between align-items-center mt-3"
        >
          <p class="text-muted">{{ $t("editPermissionsMessage") }}</p>
          <div class="d-flex align-items-center">
            <svg
              width="16"
              height="15"
              viewBox="0 0 16 15"
              class="mx-2"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M10.9922 10.1836C10.9375 10.2383 10.9375 10.293 10.9375 10.3477V13.4375H1.3125V3.8125H7.90234C7.95703 3.8125 8.01172 3.8125 8.06641 3.75781L8.94141 2.88281C9.07812 2.74609 8.96875 2.5 8.77734 2.5H1.3125C0.574219 2.5 0 3.10156 0 3.8125V13.4375C0 14.1758 0.574219 14.75 1.3125 14.75H10.9375C11.6484 14.75 12.25 14.1758 12.25 13.4375V9.47266C12.25 9.28125 12.0039 9.17188 11.8672 9.30859L10.9922 10.1836ZM15.2578 4.6875C15.8867 4.05859 15.8867 3.04688 15.2578 2.41797L14.082 1.24219C13.4531 0.613281 12.4414 0.613281 11.8125 1.24219L4.64844 8.40625L4.375 10.8945C4.29297 11.6055 4.89453 12.207 5.60547 12.125L8.09375 11.8516L15.2578 4.6875ZM12.5781 5.50781L7.49219 10.5938L5.6875 10.8125L5.90625 9.00781L10.9922 3.92188L12.5781 5.50781ZM14.3281 3.34766C14.4648 3.45703 14.4648 3.62109 14.3555 3.75781L13.5078 4.60547L11.9219 2.99219L12.7422 2.17188C12.8516 2.03516 13.043 2.03516 13.1523 2.17188L14.3281 3.34766Z"
                fill="#0074BD"
              />
            </svg>
            <AdminPopUp />
          </div>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsViewMessages") }}</p>
          <div class="d-flex align-items-center">
            <svg
              width="16"
              height="15"
              viewBox="0 0 16 15"
              class="mx-2"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M10.9922 10.1836C10.9375 10.2383 10.9375 10.293 10.9375 10.3477V13.4375H1.3125V3.8125H7.90234C7.95703 3.8125 8.01172 3.8125 8.06641 3.75781L8.94141 2.88281C9.07812 2.74609 8.96875 2.5 8.77734 2.5H1.3125C0.574219 2.5 0 3.10156 0 3.8125V13.4375C0 14.1758 0.574219 14.75 1.3125 14.75H10.9375C11.6484 14.75 12.25 14.1758 12.25 13.4375V9.47266C12.25 9.28125 12.0039 9.17188 11.8672 9.30859L10.9922 10.1836ZM15.2578 4.6875C15.8867 4.05859 15.8867 3.04688 15.2578 2.41797L14.082 1.24219C13.4531 0.613281 12.4414 0.613281 11.8125 1.24219L4.64844 8.40625L4.375 10.8945C4.29297 11.6055 4.89453 12.207 5.60547 12.125L8.09375 11.8516L15.2578 4.6875ZM12.5781 5.50781L7.49219 10.5938L5.6875 10.8125L5.90625 9.00781L10.9922 3.92188L12.5781 5.50781ZM14.3281 3.34766C14.4648 3.45703 14.4648 3.62109 14.3555 3.75781L13.5078 4.60547L11.9219 2.99219L12.7422 2.17188C12.8516 2.03516 13.043 2.03516 13.1523 2.17188L14.3281 3.34766Z"
                fill="#0074BD"
              />
            </svg>
            <AdminPopUp />
          </div>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsReplyToMessages") }}</p>
          <div class="d-flex align-items-center">
            <svg
              width="16"
              height="15"
              viewBox="0 0 16 15"
              class="mx-2"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M10.9922 10.1836C10.9375 10.2383 10.9375 10.293 10.9375 10.3477V13.4375H1.3125V3.8125H7.90234C7.95703 3.8125 8.01172 3.8125 8.06641 3.75781L8.94141 2.88281C9.07812 2.74609 8.96875 2.5 8.77734 2.5H1.3125C0.574219 2.5 0 3.10156 0 3.8125V13.4375C0 14.1758 0.574219 14.75 1.3125 14.75H10.9375C11.6484 14.75 12.25 14.1758 12.25 13.4375V9.47266C12.25 9.28125 12.0039 9.17188 11.8672 9.30859L10.9922 10.1836ZM15.2578 4.6875C15.8867 4.05859 15.8867 3.04688 15.2578 2.41797L14.082 1.24219C13.4531 0.613281 12.4414 0.613281 11.8125 1.24219L4.64844 8.40625L4.375 10.8945C4.29297 11.6055 4.89453 12.207 5.60547 12.125L8.09375 11.8516L15.2578 4.6875ZM12.5781 5.50781L7.49219 10.5938L5.6875 10.8125L5.90625 9.00781L10.9922 3.92188L12.5781 5.50781ZM14.3281 3.34766C14.4648 3.45703 14.4648 3.62109 14.3555 3.75781L13.5078 4.60547L11.9219 2.99219L12.7422 2.17188C12.8516 2.03516 13.043 2.03516 13.1523 2.17188L14.3281 3.34766Z"
                fill="#0074BD"
              />
            </svg>

            <AdminPopUp />
          </div>
        </div>
        <div
          class="w-90 d-flex justify-content-between align-items-center mt-3"
        >
          <p class="text-muted">{{ $t("editPermissionsNotifications") }}</p>
          <div class="d-flex align-items-center">
            <svg
              width="16"
              height="11"
              viewBox="0 0 16 11"
              class="mx-2"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M7.875 2.6875C7.62891 2.71484 7.24609 2.76953 7 2.82422C7.10938 3.01562 7.19141 3.37109 7.21875 3.5625C7.21875 4.41016 6.50781 5.09375 5.6875 5.09375C5.46875 5.09375 5.11328 5.01172 4.94922 4.90234C4.86719 5.14844 4.8125 5.50391 4.8125 5.75C4.8125 7.44531 6.17969 8.8125 7.875 8.8125C9.57031 8.8125 10.9375 7.44531 10.9375 5.75C10.9375 4.08203 9.57031 2.6875 7.875 2.6875ZM15.6406 5.36719C14.1641 2.46875 11.2109 0.5 7.875 0.5C4.51172 0.5 1.55859 2.46875 0.0820312 5.36719C0.0273438 5.47656 0 5.64062 0 5.77734C0 5.88672 0.0273438 6.05078 0.0820312 6.16016C1.55859 9.05859 4.51172 11 7.875 11C11.2109 11 14.1641 9.05859 15.6406 6.16016C15.6953 6.05078 15.7227 5.88672 15.7227 5.75C15.7227 5.64062 15.6953 5.47656 15.6406 5.36719ZM7.875 9.6875C5.16797 9.6875 2.67969 8.18359 1.36719 5.75C2.67969 3.31641 5.16797 1.8125 7.875 1.8125C10.5547 1.8125 13.043 3.31641 14.3555 5.75C13.043 8.18359 10.5547 9.6875 7.875 9.6875Z"
                fill="#0074BD"
              />
            </svg>

            <AdminPopUp />
          </div>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsView") }}</p>
          <div class="d-flex align-items-center">
            <svg
              width="16"
              height="11"
              viewBox="0 0 16 11"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M7.875 2.6875C7.62891 2.71484 7.24609 2.76953 7 2.82422C7.10938 3.01562 7.19141 3.37109 7.21875 3.5625C7.21875 4.41016 6.50781 5.09375 5.6875 5.09375C5.46875 5.09375 5.11328 5.01172 4.94922 4.90234C4.86719 5.14844 4.8125 5.50391 4.8125 5.75C4.8125 7.44531 6.17969 8.8125 7.875 8.8125C9.57031 8.8125 10.9375 7.44531 10.9375 5.75C10.9375 4.08203 9.57031 2.6875 7.875 2.6875ZM15.6406 5.36719C14.1641 2.46875 11.2109 0.5 7.875 0.5C4.51172 0.5 1.55859 2.46875 0.0820312 5.36719C0.0273438 5.47656 0 5.64062 0 5.77734C0 5.88672 0.0273438 6.05078 0.0820312 6.16016C1.55859 9.05859 4.51172 11 7.875 11C11.2109 11 14.1641 9.05859 15.6406 6.16016C15.6953 6.05078 15.7227 5.88672 15.7227 5.75C15.7227 5.64062 15.6953 5.47656 15.6406 5.36719ZM7.875 9.6875C5.16797 9.6875 2.67969 8.18359 1.36719 5.75C2.67969 3.31641 5.16797 1.8125 7.875 1.8125C10.5547 1.8125 13.043 3.31641 14.3555 5.75C13.043 8.18359 10.5547 9.6875 7.875 9.6875Z"
                fill="#0074BD"
              />
            </svg>
            <AdminPopUp />
          </div>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <div>
            <p class="mb-1">{{ $t("editPermissionsClientStatus") }}</p>
            <p class="mb-0 fs-10 text-muted">
              {{ $t("editPermissionsMoreDetails") }}
            </p>
          </div>
          <div class="d-flex align-items-center">
            <svg
              width="16"
              height="15"
              class="mx-2"
              viewBox="0 0 16 15"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M10.9922 10.1836C10.9375 10.2383 10.9375 10.293 10.9375 10.3477V13.4375H1.3125V3.8125H7.90234C7.95703 3.8125 8.01172 3.8125 8.06641 3.75781L8.94141 2.88281C9.07812 2.74609 8.96875 2.5 8.77734 2.5H1.3125C0.574219 2.5 0 3.10156 0 3.8125V13.4375C0 14.1758 0.574219 14.75 1.3125 14.75H10.9375C11.6484 14.75 12.25 14.1758 12.25 13.4375V9.47266C12.25 9.28125 12.0039 9.17188 11.8672 9.30859L10.9922 10.1836ZM15.2578 4.6875C15.8867 4.05859 15.8867 3.04688 15.2578 2.41797L14.082 1.24219C13.4531 0.613281 12.4414 0.613281 11.8125 1.24219L4.64844 8.40625L4.375 10.8945C4.29297 11.6055 4.89453 12.207 5.60547 12.125L8.09375 11.8516L15.2578 4.6875ZM12.5781 5.50781L7.49219 10.5938L5.6875 10.8125L5.90625 9.00781L10.9922 3.92188L12.5781 5.50781ZM14.3281 3.34766C14.4648 3.45703 14.4648 3.62109 14.3555 3.75781L13.5078 4.60547L11.9219 2.99219L12.7422 2.17188C12.8516 2.03516 13.043 2.03516 13.1523 2.17188L14.3281 3.34766Z"
                fill="#0074BD"
              />
            </svg>

            <AdminPopUp />
          </div>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <div>
            <p class="mb-1">{{ $t("editPermissionsAccountStatus") }}</p>
            <p class="mb-0 fs-10 text-muted">
              {{ $t("editPermissionsMoreDetails") }}
            </p>
          </div>
          <div class="d-flex align-items-center">
            <svg
              width="16"
              height="15"
              viewBox="0 0 16 15"
              class="mx-2"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M10.9922 10.1836C10.9375 10.2383 10.9375 10.293 10.9375 10.3477V13.4375H1.3125V3.8125H7.90234C7.95703 3.8125 8.01172 3.8125 8.06641 3.75781L8.94141 2.88281C9.07812 2.74609 8.96875 2.5 8.77734 2.5H1.3125C0.574219 2.5 0 3.10156 0 3.8125V13.4375C0 14.1758 0.574219 14.75 1.3125 14.75H10.9375C11.6484 14.75 12.25 14.1758 12.25 13.4375V9.47266C12.25 9.28125 12.0039 9.17188 11.8672 9.30859L10.9922 10.1836ZM15.2578 4.6875C15.8867 4.05859 15.8867 3.04688 15.2578 2.41797L14.082 1.24219C13.4531 0.613281 12.4414 0.613281 11.8125 1.24219L4.64844 8.40625L4.375 10.8945C4.29297 11.6055 4.89453 12.207 5.60547 12.125L8.09375 11.8516L15.2578 4.6875ZM12.5781 5.50781L7.49219 10.5938L5.6875 10.8125L5.90625 9.00781L10.9922 3.92188L12.5781 5.50781ZM14.3281 3.34766C14.4648 3.45703 14.4648 3.62109 14.3555 3.75781L13.5078 4.60547L11.9219 2.99219L12.7422 2.17188C12.8516 2.03516 13.043 2.03516 13.1523 2.17188L14.3281 3.34766Z"
                fill="#0074BD"
              />
            </svg>

            <AdminPopUp />
          </div>
        </div>
        <div
          class="w-90 d-flex justify-content-between align-items-center mt-3"
        >
          <p class="text-muted">
            {{ $t("editPermissionsEmployeesAndUserManagment") }}
          </p>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <p class="mb-0">{{ $t("editPermissionsCreateUser") }}</p>
          <div class="d-flex align-items-center">
            <svg
              width="19"
              height="15"
              viewBox="0 0 19 15"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18.3359 13.6289L1.98438 0.859375C1.90234 0.804688 1.79297 0.75 1.71094 0.75C1.57422 0.75 1.4375 0.832031 1.35547 0.914062L1.08203 1.26953C1.02734 1.32422 0.972656 1.46094 0.972656 1.54297C0.972656 1.65234 1.05469 1.81641 1.16406 1.87109L17.5156 14.668C17.5703 14.7227 17.6797 14.75 17.7891 14.75C17.8984 14.75 18.0352 14.6953 18.1172 14.5859L18.3906 14.2578C18.4453 14.2031 18.5 14.0664 18.5 13.9844C18.5 13.875 18.418 13.7109 18.3359 13.6289ZM9.09375 4.76953L12.7852 7.64062C12.7305 6 11.3906 4.6875 9.75 4.6875C9.55859 4.71484 9.28516 4.74219 9.09375 4.76953ZM10.3789 10.7578L6.6875 7.88672C6.74219 9.52734 8.08203 10.8125 9.75 10.8125C9.91406 10.8125 10.1875 10.7852 10.3789 10.7578ZM9.75 3.8125C12.4297 3.8125 14.918 5.31641 16.2305 7.75C15.957 8.26953 15.4375 9.00781 15.0273 9.41797L16.0664 10.2109C16.5586 9.69141 17.1875 8.78906 17.5156 8.16016C17.5703 8.05078 17.625 7.88672 17.625 7.75C17.625 7.64062 17.5703 7.47656 17.5156 7.36719C16.0391 4.46875 13.0859 2.5 9.75 2.5C8.73828 2.5 7.78125 2.69141 6.87891 3.01953L8.13672 4.03125C8.65625 3.89453 9.20312 3.8125 9.75 3.8125ZM9.75 11.6875C7.04297 11.6875 4.55469 10.1836 3.24219 7.75C3.51562 7.25781 4.03516 6.51953 4.44531 6.10938L3.40625 5.31641C2.91406 5.83594 2.28516 6.73828 1.95703 7.36719C1.90234 7.47656 1.875 7.64062 1.875 7.77734C1.875 7.88672 1.90234 8.05078 1.95703 8.16016C3.43359 11.0586 6.38672 13 9.75 13C10.7344 13 11.6914 12.8086 12.5938 12.5078L11.3359 11.4961C10.8164 11.6328 10.2695 11.6875 9.75 11.6875Z"
                fill="#0074BD"
              />
            </svg>

            <AdminPopUp />
          </div>
        </div>
        <div
          class="w-70 mt-3 d-flex justify-content-between align-items-center"
        >
          <div>
            <p class="mb-1">{{ $t("editPermissionsManageUser") }}</p>
            <p class="mb-0 fs-10 text-muted">
              {{ $t("editPermissionsActivate") }}
            </p>
          </div>
          <div class="d-flex align-items-center">
            <svg
              width="16"
              height="11"
              viewBox="0 0 16 11"
              fill="none"
              class="mx-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M7.875 2.6875C7.62891 2.71484 7.24609 2.76953 7 2.82422C7.10938 3.01562 7.19141 3.37109 7.21875 3.5625C7.21875 4.41016 6.50781 5.09375 5.6875 5.09375C5.46875 5.09375 5.11328 5.01172 4.94922 4.90234C4.86719 5.14844 4.8125 5.50391 4.8125 5.75C4.8125 7.44531 6.17969 8.8125 7.875 8.8125C9.57031 8.8125 10.9375 7.44531 10.9375 5.75C10.9375 4.08203 9.57031 2.6875 7.875 2.6875ZM15.6406 5.36719C14.1641 2.46875 11.2109 0.5 7.875 0.5C4.51172 0.5 1.55859 2.46875 0.0820312 5.36719C0.0273438 5.47656 0 5.64062 0 5.77734C0 5.88672 0.0273438 6.05078 0.0820312 6.16016C1.55859 9.05859 4.51172 11 7.875 11C11.2109 11 14.1641 9.05859 15.6406 6.16016C15.6953 6.05078 15.7227 5.88672 15.7227 5.75C15.7227 5.64062 15.6953 5.47656 15.6406 5.36719ZM7.875 9.6875C5.16797 9.6875 2.67969 8.18359 1.36719 5.75C2.67969 3.31641 5.16797 1.8125 7.875 1.8125C10.5547 1.8125 13.043 3.31641 14.3555 5.75C13.043 8.18359 10.5547 9.6875 7.875 9.6875Z"
                fill="#0074BD"
              />
            </svg>
            <AdminPopUp />
          </div>
        </div>
      </div>
      <div class="w-33-resp py-3 d-flex flex-column align-items-center mt-3">
        <div
          class="w-25-resp align-self-start d-flex align-items-center justify-content-around"
        >
          <svg
            width="42"
            height="42"
            viewBox="0 0 42 42"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <circle cx="21" cy="21" r="21" fill="#0074BD" />
            <path
              d="M24.4766 23.375C23.3438 23.375 22.8359 24 21 24C19.125 24 18.6172 23.375 17.4844 23.375C14.5938 23.375 12.25 25.7578 12.25 28.6484V29.625C12.25 30.6797 13.0703 31.5 14.125 31.5H27.875C28.8906 31.5 29.75 30.6797 29.75 29.625V28.6484C29.75 25.7578 27.3672 23.375 24.4766 23.375ZM27.875 29.625H14.125V28.6484C14.125 26.7734 15.6094 25.25 17.4844 25.25C18.0703 25.25 18.9688 25.875 21 25.875C22.9922 25.875 23.8906 25.25 24.4766 25.25C26.3516 25.25 27.875 26.7734 27.875 28.6484V29.625ZM21 22.75C24.0859 22.75 26.625 20.25 26.625 17.125C26.625 14.0391 24.0859 11.5 21 11.5C17.875 11.5 15.375 14.0391 15.375 17.125C15.375 20.25 17.875 22.75 21 22.75ZM21 13.375C23.0312 13.375 24.75 15.0938 24.75 17.125C24.75 19.1953 23.0312 20.875 21 20.875C18.9297 20.875 17.25 19.1953 17.25 17.125C17.25 15.0938 18.9297 13.375 21 13.375Z"
              fill="white"
            />
          </svg>

          <p class="mb-0">{{ $t("editPermissionsUsers") }}</p>
        </div>
        <div
          class="mt-4 w-100 align-self-start d-flex align-items-center justify-content-around"
        >
          <div class="d-flex flex-column">
            <button class="outline-none">
              <svg
                width="14"
                height="15"
                viewBox="0 0 14 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M10.5 7.3125C10.5 7.14844 10.3359 6.98438 10.1719 6.98438H7.76562V4.57812C7.76562 4.41406 7.60156 4.25 7.4375 4.25H6.5625C6.37109 4.25 6.23438 4.41406 6.23438 4.57812V6.98438H3.82812C3.63672 6.98438 3.5 7.14844 3.5 7.3125V8.1875C3.5 8.37891 3.63672 8.51562 3.82812 8.51562H6.23438V10.9219C6.23438 11.1133 6.37109 11.25 6.5625 11.25H7.4375C7.60156 11.25 7.76562 11.1133 7.76562 10.9219V8.51562H10.1719C10.3359 8.51562 10.5 8.37891 10.5 8.1875V7.3125ZM13.7812 7.75C13.7812 4.00391 10.7461 0.96875 7 0.96875C3.25391 0.96875 0.21875 4.00391 0.21875 7.75C0.21875 11.4961 3.25391 14.5312 7 14.5312C10.7461 14.5312 13.7812 11.4961 13.7812 7.75ZM12.4688 7.75C12.4688 10.7852 10.0078 13.2188 7 13.2188C3.96484 13.2188 1.53125 10.7852 1.53125 7.75C1.53125 4.74219 3.96484 2.28125 7 2.28125C10.0078 2.28125 12.4688 4.74219 12.4688 7.75Z"
                  fill="#0074BD"
                />
              </svg>
            </button>
            <button class="outline-none">
              <svg
                width="14"
                height="13"
                viewBox="0 0 14 13"
                fill="none"
                class="opacity-50"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M7.26172 12.7109L7.80859 12.1914C7.94531 12.0547 7.94531 11.8359 7.80859 11.7266L3.57031 7.46094H12.9219C13.0859 7.46094 13.25 7.32422 13.25 7.13281V6.36719C13.25 6.20312 13.0859 6.03906 12.9219 6.03906H3.57031L7.80859 1.80078C7.94531 1.69141 7.94531 1.47266 7.80859 1.33594L7.26172 0.816406C7.15234 0.679688 6.93359 0.679688 6.79688 0.816406L1.08203 6.53125C0.945312 6.66797 0.945312 6.85938 1.08203 6.99609L6.79688 12.7109C6.93359 12.8477 7.15234 12.8477 7.26172 12.7109Z"
                  fill="#FF3A67"
                />
              </svg>
            </button>
          </div>
          <div class="d-flex flex-column justify-content-center">
            <p class="mb-0 mt-1">{{ $t("editPermissionsMoveUsers") }}</p>
            <p class="mb-0 mt-2 pink-text opacity-50">
              {{ $t("editPermissionsMoveUser") }}
            </p>
          </div>
        </div>
        <div
          class="mt-4 w-100 align-self-start d-flex align-items-center justify-content-around"
        >
          <div class="w-75-resp">
            <svg
              width="14"
              height="15"
              viewBox="0 0 14 15"
              fill="none"
              class="position-absolute mx-2 mt-2"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M13.8906 13.5742L10.582 10.2656C10.5 10.2109 10.418 10.1562 10.3359 10.1562H9.98047C10.8281 9.17188 11.375 7.85938 11.375 6.4375C11.375 3.32031 8.80469 0.75 5.6875 0.75C2.54297 0.75 0 3.32031 0 6.4375C0 9.58203 2.54297 12.125 5.6875 12.125C7.10938 12.125 8.39453 11.6055 9.40625 10.7578V11.1133C9.40625 11.1953 9.43359 11.2773 9.48828 11.3594L12.7969 14.668C12.9336 14.8047 13.1523 14.8047 13.2617 14.668L13.8906 14.0391C14.0273 13.9297 14.0273 13.7109 13.8906 13.5742ZM5.6875 10.8125C3.25391 10.8125 1.3125 8.87109 1.3125 6.4375C1.3125 4.03125 3.25391 2.0625 5.6875 2.0625C8.09375 2.0625 10.0625 4.03125 10.0625 6.4375C10.0625 8.87109 8.09375 10.8125 5.6875 10.8125Z"
                fill="black"
                fill-opacity="0.55"
              />
            </svg>
            <input
              class="w-100 rounded-lg border px-15 outline-none py-1"
              :placeholder="`${$t('editPermissionsSearch')}`"
            />
          </div>
        </div>
        <div
          class="mt-5 w-100 align-self-start d-flex align-items-center justify-content-around"
        >
          <div class="w-75-resp">
            <b-form-checkbox
              id="checkbox-fullname"
              name="checkbox-1"
              value="accepted"
              unchecked-value="not_accepted"
            >
              {{ $t("editPermissionsFirstLast") }}
            </b-form-checkbox>
          </div>
        </div>
        <div
          class="mt-3 w-100 align-self-start d-flex align-items-center justify-content-around"
        >
          <div class="w-75-resp">
            <b-form-checkbox
              id="checkbox-fullname1"
              name="checkbox-1"
              value="accepted"
              unchecked-value="not_accepted"
            >
              {{ $t("editPermissionsFirstLast") }}
            </b-form-checkbox>
          </div>
        </div>
        <div
          class="mt-3 w-100 align-self-start d-flex align-items-center justify-content-around"
        >
          <div class="w-75-resp">
            <b-form-checkbox
              id="checkbox-fullname2"
              name="checkbox-1"
              value="accepted"
              unchecked-value="not_accepted"
            >
              {{ $t("editPermissionsFirstLast") }}
            </b-form-checkbox>
          </div>
        </div>
        <div
          class="mt-3 w-100 align-self-start d-flex align-items-center justify-content-around"
        >
          <div class="w-75-resp">
            <b-form-checkbox
              id="checkbox-fullname3"
              name="checkbox-1"
              value="accepted"
              unchecked-value="not_accepted"
            >
              {{ $t("editPermissionsFirstLast") }}
            </b-form-checkbox>
          </div>
        </div>
        <div
          class="mt-3 w-100 align-self-start d-flex align-items-center justify-content-around"
        >
          <div class="w-75-resp">
            <b-form-checkbox
              id="checkbox-fullname4"
              name="checkbox-1"
              value="accepted"
              unchecked-value="not_accepted"
            >
              {{ $t("editPermissionsFirstLast") }}
            </b-form-checkbox>
          </div>
        </div>
        <div
          class="mt-3 w-100 align-self-start d-flex align-items-center justify-content-around"
        >
          <div class="w-75-resp">
            <b-form-checkbox
              id="checkbox-fullname5"
              name="checkbox-1"
              value="accepted"
              unchecked-value="not_accepted"
            >
              {{ $t("editPermissionsFirstLast") }}
            </b-form-checkbox>
          </div>
        </div>
      </div>
    </div>
  </b-modal>
</template>
<script>
import AdminPopUp from "../admin/popUp";
export default {
  name: "EditPermissions",
  data() {
    return {
      visible: true,
    };
  },
  components: {
    AdminPopUp,
  },
  methods: {
    handleClose() {
      this.$router.push({ path: "/admin/dashboard/employees" });
    },
  },
};
</script>

<style>
.header-class2 {
  border: 0px !important;
  padding: 0 10px !important;
  position: absolute;
  right: 0;
  z-index: 2;
}
.body-class2 {
  padding: 0px 5px !important;
}
.pink-text {
  color: #ff3a67;
}
.opacity-50 {
  opacity: 0.5;
}
.w-25-resp {
  width: 50%;
}
.w-33-resp {
  width: 100%;
}
@media screen and (min-width: 992px) and (max-width: 1200px) {
  .w-33-resp {
    width: 45%;
  }
  .w-25-resp {
    width: 35%;
  }
}
@media screen and (min-width: 1200px) {
  .w-33-resp {
    width: 45%;
  }
  .w-25-resp {
    width: 50%;
  }
}
</style>